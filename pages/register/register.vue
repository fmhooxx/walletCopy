...
export default {
  ...
  computed: {
    isActives () {
      if (this.phone != '' && this.code != '' && this.pwd != '' && this.InCode != '') {
        return true
      } else {
        return false
      }
  ...
  onLoad (options) {
    if (options.referrer != '' && options.referrer != undefined) {
      this.InCode = options.referrer
  ...
  methods: {
    InCodeBlur () {
      this.InCode = this.InCode.replace(/\s+/g, '')
    },
    pwdBlur () {
    ...
    async goReturn () {
      ...
      if (this.InCode.length == 0) {
        return this.$api.msg('请输入邀请码')
      }
      let data = {
        username: this.phone,
        ...
        password: this.pwd,
        authCode: this.code,
        referrer: this.InCode
      }
      let res = await this.$api.api.register(data)
      if (res.data.code == 200) {
        this.$api.msg('注册成功')
        if (this.referrer != '' && this.referrer != undefined) {
          return uni.reLaunch({
